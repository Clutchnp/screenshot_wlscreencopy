...
static void flags_recieved (void* data,
struct zwlr_screencopy_frame_v1* zwlr_screencopy_frame_v1,
uint32_t flags);
...
uint32_t stride) {
    ...
    fdata->shm_data =
    mmap (NULL, fdata->size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
uint32_t tv_nsec) {
    ...
    process_pixels (fdata->shm_data, fdata->width, fdata->height, fdata->stride,
...
uint32_t version) {
    if (strcmp (interface, wl_compositor_interface.name) == 0)
        ...
        screencopy_manager =
        wl_registry_bind (registry, id, &zwlr_screencopy_manager_v1_interface, 3);
